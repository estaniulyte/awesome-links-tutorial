...
export const Link = objectType({
  name: 'Link',
  definition(t) { //define fields (table)
    t.string('id');
    t.string('title');
    t.string('url');
    ...
    t.string('imageUrl');
    t.string('category');
    t.list.field('users', { //array of type users
      type: User,
...
// client sends a request, than can either be the first req,
// send another req, which will contain a cursor
  ...
  definition(t) {
    t.field('links', {
      ...
      async resolve(_, args, ctx) {
        ...
        if (args.after) {
          //check if tehre is a cursor as the argument
        ...
        if (queryResults.length > 0) {
          ...
          const result = {
            pageInfo: {
              endCursor: myCursor,
              hasNextPage: secondQueryResults.length >= args.first, // if the number of items requested is greater than the response of the secondary query, we have another page
            },
